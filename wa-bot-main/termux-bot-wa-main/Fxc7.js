	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `@${num.split('@')[0]} \nBem Vindo ao Grupo*${mdata.subject}*\napresente-se com nome e idade! Leia a descrição para não ser banido!!`
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '*Sucesso...*',
				error: {
				 bug: '*ocorreu um erro, tente entrar em contato com o proprietário para relatar o erro*',
				stick: ' *falhou, ocorreu um erro ao converter a imagem em adesivo*\n*tente novamente com a foto que foi enviada*',
				Iv: '*Desculpe link invalido!!*'
				},
			only: {
				group: '*DESCULPE, ESTE PEDIDO SÓ PODE SER USADO EM GRUPO!*',
				benned: '*DESCULPE, SEU NÚMERO ESTÁ PROIBIDO, ENTRE EM CONTATO COM O PROPRIETÁRIO PARA SER DESBANIDO*',
...
if (isGroup) {
		...
		setTimeout( () => {
		frhan.sendMessage(from, `Desculpe ${name} Não posso entrar no grupo porque membro do grupo ${groupMetadata.subject} Não atende ao limite de membros\n\nminimo ${memberLimit}`, text)
		}, 0)
	}
		...
		setTimeout( () => {
		reply(`Link de grupo detectado, desculpe *${pushname2}* você será chutado`)
		}, 0)
	}
		if (txt.length > 500){
        if (!isGroup) return
		...
		.then(() => {
		frhan.sendMessage(from, `*「 ANTI PALAVRÃO 」*\nVocê foi chutado por ser rude!`, text ,{quoted: mek})
		}).catch(() => frhan.sendMessage(from, `Felizmente Iren não é um administrador, se o administrador já for Irenn, chute!`, text , {quoted : mek}))
		} else {
...
const checkLimit = (sender) => {
			...
			if (limitCounts <= 0) return frhan.sendMessage(from,`Sua solicitação de limite acabou `, text, { quoted: mek})
...
return new Promise(resolve => setTimeout(resolve, ms));
		...
		case 'mutualan':
			...
			if (isGroup) return  reply( 'Este comando não pode ser usado em grupos, por favor, use-o em um bot de bate-papo privado')
		...
		break
			...
			case 'cekpremium':
				...
				reply(`*「 PREMIUM EXPIRED 」*\n\n➸ *ID*: ${sender.split('@')[0]}\n➸ *Premium ACABA*: ${cekExp.days} Dias(s) ${cekExp.hours} Hrs(s) ${cekExp.minutes} minutos(s)`)
			break
			case 'wame':
...
// end fitur simple
			...
			case 'infobot':
				...
				case 'block':
				 frhan.updatePresence(from, Presence.composing)
					...
					frhan.sendMessage(from, `Pedido recebido, bloqueando ${body.slice(7)}@c.us`, text)
					break
				case 'unblock':
                    if (isBanned) return reply(mess.only.benned)
				if (!isUser) return reply(mess.only.userB)
					...
					frhan.sendMessage(from, `Pedido recebido, desbloquear ${body.slice(9)}@c.us`, text)
				    break
				case 'readmore':
					if (isBanned) return reply(mess.only.benned)
					if (!isUser) return reply(mess.only.userB)
				...
				case 'sticker':
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('adityaXCode7', 'Jangan Lupa Donasi')} ${ran} -o ${ran}`, async (error) => {
									...
									await costum(fs.readFileSync(ran), sticker, adityaGans, ` ~ Aqui está o adesivo`)
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
								})
							})
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('adityaXCode7', 'Jangan Lupa Donasi')} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									await costum(fs.readFileSync(ran), sticker, adityaGans, `~ Aqui está um Gif Sticker`)
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
								})
							})
			...
			break
				...
				if (!isGroup) return reply(mess.only.group)
				if (!isOwner) return reply(mess.only.ownerB)
				anu = await frhan.groupLeave(from, `Bye  *${groupMetadata.subject}*`, groupId)
				break
				case 'setname':
				if (isBanned) return reply(mess.only.benned)
				if (!isUser) return reply(mess.only.userB)
				...
				frhan.sendMessage(from, `\`\`\`✓NOME SETADO\`\`\` *${body.slice(9)}*`, text, {quoted: mek})
			break
			case 'setdesc':
				if (isBanned) return reply(mess.only.benned)
				if (!isUser) return reply(mess.only.userB)
				...
				frhan.sendMessage(from, `\`\`\`✓DESC SETADO\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, {quoted: mek})
			break
			case 'tts':
				if (isBanned) return reply(mess.only.benned)
				if (!isUser) return reply(mess.only.userB)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return frhan.sendMessage(from, 'Qual é o código do idioma, mano? Se você não sabe o código do idioma, basta digitá-lo *${prefix}bahasa*', text, {quoted: mek})
			...
			case 'nsfwloli':
				...
				frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Não faça os ingredientes para comer'})
				} catch (e) {
			...
			break
			    ...
			    case 'nsfwneko':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nianji '})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
				...
				reply(mess.wait)
				try {
					if (args.length < 1) return reply('Que acordes de música você está procurando???')
					tels = body.slice(7)
...
// akhir fitur anime
				...
				reply(mess.wait)
					...
					if (anu.error) return reply(anu.error)
					frhan.sendMessage(from, '[ WAIT ] Processando... O link é apenas do Google bro, portanto, pode ser baixado', text, {quoted: mek})
			...
			case 'hekerbucin':
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return reply('Quais filmes você está procurando?')
				try {
				reply(mess.wait)
...
//creator
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return reply(`${name} O que você deve escrever mano???`)
				reply(mess.wait)
				teks = `${body.slice(7)}`
			...
			const	textnya = teks.split("/")[2];
				...
				if (args.length < 1) {
						...
						return reply('O texto também mano')
					}
					reply(mess.wait)
			...
			case 'ttp':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return reply('o que você quer usar como adesivo de texto??')
				reply(mess.wait)
				try {
					ranp = getRandom('.png')
					rano = getRandom('.webp')
...
//akhir kreator
		    ...
		    try {
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Marcado por *${pushname2}* ✪══`+ teks +'╚═══〘 BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    try {
			    if (isBanned) return reply(mess.only.benned)
			    ...
			    reply(mess.wait)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`\`\`\`✓Abrindo um Grupo com Sucesso\`\`\` *${groupMetadata.subject}*`)
						frhan.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`\`\`\`✓Fechando Grupo com Sucesso\`\`\` *${groupMetadata.subject}*`)
				...
				case 'clearall':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							frhan.sendMessage(_.jid, bcgc, image, {caption: `*「 BROADCAST 」*\n*Grupo* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							frhan.sendMessage(_.jid, bc, image, {caption: `[  Broadcast ]\n\n${body.slice(4)}`})
						}
				...
				case 'kicktime':
					...
					setTimeout( () => {
					frhan.sendMessage(from, `Kick @${mentioned[0].split('@')[0]}`, text) // ur cods
					}, 5000)
					setTimeout( () => {
					frhan.sendMessage(from, 'Legal para conseguir comida :D', text)
					}, 2500)
					setTimeout( () => {
					reply('Pedido Recebido :D')
					}, 0)
					break
				case 'promote':
				if (isBanned) return reply(mess.only.benned)
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag o alvo que quer ser um administrador!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, você se torna administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido Recebido, @${mentioned[0].split('@')[0]}Você é um administrador do grupo *${groupMetadata.subject}*`, mentioned, true)
						frhan.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o alvo que você não quer que seja um administrador!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, rebaixar: @${mentioned[0].split('@')[0]} Virou Membro`, mentioned, true)
						frhan.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'adminlist':
				...
				case 'antivirtex':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('toque em para ativar')
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`Ativar com sucesso o modo anti-virtex no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						if (!isAntiVirtex) return reply('O modo antivirtex foi desativado')
						_antivirtex.splice(from, 1)
						fs.writeFileSync('./database/json/antivirtex.json', JSON.stringify(_antivirtex))
						reply(`\`\`\`✓Desative o modo anti-virtex com sucesso no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if ((args[0]) === 'on') {
						...
						if (!isSimi) return reply('O modo Simi já está desligado')
						_samih.splice(from, 1)
						fs.writeFileSync('./database/json/simi.json', JSON.stringify(_samih))
						reply(`\`\`\`✓Desativar com sucesso o modo xiaomi no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On untuk mengaktifkan, Off untuk menonaktifkan')
					}
					break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						...
						if (!isNsfw) return reply('O modo Nsfw está desligado antes')
						_nsfw.splice(from, 1)
						fs.writeFileSync('./database/json/nsfw.json', JSON.stringify(_nsfw))
						reply(`\`\`\`✓Desativar com sucesso o modo nsfw no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On untuk mengaktifkan, Off untuk menonaktifkan')
					}
					break
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (isAnime) return reply('O modo anime está ativo')
						_anime.push(from)
						...
						if (!isAnime) return reply('O modo anime estava desativado antes')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativar com sucesso o recurso de boas-vindas no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						if (!isWelkom) return reply('Mode Welcome Sudah Off Sebelumnya')
						_welkom.splice(from, 1)
						fs.writeFileSync('./database/json/welkom.json', JSON.stringify(_welkom))
						reply(`\`\`\`✓Desativar com sucesso o recurso de boas-vindas no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On untuk mengaktifkan, Off untuk menonaktifkan')
					}
					break
				...
				if (!isGroup) return reply(mess.only.group)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Ativar com sucesso o recurso anti-link no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						if (!isAntiLink) return reply('anti-link foi desligado antes')
						_antilink.splice(from, 1)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(_antilink))
						reply(`\`\`\`✓Desativar com sucesso o recurso anti-link no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('on untuk mengaktifkan, off untuk menonaktifkan')
					}
					break
				...
				if (!isGroup) return reply(mess.only.group)
					...
					if ((args[0]) === 'on') {
						if (isBadWord) return reply('anti badword está ativo')
						_badword.push(from)
						...
						if (!isBadWord) return reply('anti badword foi desativado antes')
						_badword.splice(from, 1)
						fs.writeFileSync('./database/json/badword.json', JSON.stringify(_badword))
						reply(`\`\`\`✓Desativar com sucesso o recurso anti-badword no grupo\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('on untuk mengaktifkan, off untuk menonaktifkan')
					}
					break
...
////////////
         ...
         if (isBanned) return reply(mess.only.benned)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return reply('Que fotos você está procurando????')
					pinte = body.slice(11)
...
// premium user
         ...
         try {
          ...
          case 'snack':
			...
			if (isBanned) return reply(mess.only.benned)
               ...
               if (anu.error) return reply(anu.error)
                 sck = `「 *SNACK VIDEO DOWNLOADER* 」\n\n*• Format:* ${anu.format}\n*• Size:* ${anu.size}\n\n*ESPERE UM MAIS ENVIADO POR FAVOR, NÃO SPAM *`
       ...
       case 'ytmp4':
					...
					if (anu.error) return reply(anu.error)
					ytt = `╭─「 *YOUTUBE MP4 DOWNLOADER* 」\n│\n│• *Título:* ${anu.title}\n│• *Tamanho:* ${anu.filesize}\n│\n│ Espere um minuto 1 minuto talvez um pouco longo  Porque baixa Video\n╰─────────────────────`
				...
				if (!isUser) return reply(mess.only.userB)
					...
					case 'tovideo':
					  ...
					  exec(`ffmpeg -i ${anum} ${ran}`, (err) => {
					    fs.unlinkSync(anum)
					    if (err) return reply('Falha ao converter adesivo em vídeo')
					    buffer = fs.readFileSync(ran)
		...
		break
			...
			case 'randombts':
				...
				frhan.sendMessage(from, buff, image, {quoted: mek, caption: 'BTS.....'})
					...
					await limitAdd(sender)
			case 'antivirtexx':
			await costum(antivirtexx(), text, adityaGans, `Cadela Dahh🔥`)
			break
			case 'addstatus':
					if (!isOwner) return reply(mess.only.ownerB)
